...
Height `800`. The trainer sits in a rounded black bumper on a transparent surround, so it floats as a contained card on the Canvas page rather than filling the column width. The tallest state (Practice on the hardest tier, ~788px) drives the height. The device scales ~14% on desktop and drops to 100% below 600px so it still fits a phone.

## Build state

**v0.9 — shipped. Piano + strings + two synth voices, Practice + Challenge + Survival (with waves + voice), shared leaderboard live, floating bumper frame.**

Working: full audio engine, six difficulty tiers, harmonic / ascending / descending, root transposition on the top tiers, octave-reduced compounds, speed-bonus scoring, Practice with direction toggles, 20-question Challenge run, hidden Survival mode with wave counter and voice announcement, arcade high-score entry (alphanumeric + symbols), shared Challenge (top 10) and Survival (top 3) leaderboards backed by a Google Sheet.

Pending:
- **Guitar**: chip present but disabled, awaiting guitar samples (same format as piano/strings).
- **Strings loudness**: matched by trim length, not loudness-matched to piano.
- **Apps Script symbol regex**: for leaderboard symbols to persist to the sheet, the script's initials sanitiser must allow them (see Leaderboard note).
...
- **Survival** (hidden): easter-egg trigger — click the DIFFICULTY label 16 times within 8 seconds. Hot red palette, voice announcement on entry, fastest content (hardest tier, pinned), single listen, reflex-speed scoring ladder, three lives. A wrong answer costs a life; zero lives ends it. **A wave = five correct in a row** (a miss resets progress toward the next wave but banks completed waves). Top 3 board. Third button is EXIT.
...
Keyboard (undocumented in-widget, for a separate instructions sheet): number keys 1-5 select sound; `[` / `]` move difficulty; Space replays; Enter advances; the Ableton QWERTY row (A W S E D F T G Y H U J K) answers intervals relative to A = root. Initials entry: type directly, or arrows to spin a letter, Enter to confirm.

## Leaderboard

Backed by a Google Apps Script Web App bound to a Sheet (owner-managed). The endpoint URL lives in the `LEADERBOARD_URL` constant near the top of the script in `index.html` — a public read/append endpoint by design, not a secret. GET returns the top N per board (JSONP fallback for cross-origin reads); POST appends one sanitised row. If `LEADERBOARD_URL` is blanked, the board falls back to seeded local demo scores.

Initials accept letters, numbers, and the symbols `. ! ? # * @ & + - _ = / : $ % ^ ~`. The board render HTML-escapes all initials, so no input can break the display. For symbols to persist, the Apps Script sanitiser must allow the same set:

```javascript
.replace(/[^A-Z0-9.!?#*@&+_=\/:$%~^-]/g, '')
```

(then redeploy a new version of the Web App).

**Known test:** the outbound call must still be verified from inside a real Canvas page — Canvas CSP is stricter than a plain browser tab and could block the request to `script.google.com`. The widget degrades gracefully if blocked (board runs empty, entry still works, nothing breaks). Test on the day it's embedded.

## Technical notes

- Single self-contained HTML file, ~5.8 MB, Web Audio API only, no libraries.
...
- Two synth voices (sine, saw) generated live; sine and saw are -10 dB relative to raw oscillator level.
- Survival voice announcement is the user's WAV embedded untouched (stereo, original level), played to output at unity gain. Falls back to a synth sting if it fails to decode.
...
Two sampled instruments plus the voice sit near the practical base64 ceiling. Adding a third instrument (guitar) should move audio to separate files fetched same-origin from GitHub Pages, sidestepping the size limit.

## Open TODOs

- Verify the leaderboard call from inside real Canvas (CSP).
- Update the Apps Script initials regex so leaderboard symbols persist.
...
2026-07-17 — v0.9 shipped. Survival waves now mean five correct in a row (was counting every correct). Also folded in: real Survival voice announcement (embedded as-is, stereo), and leaderboard initials entry expanded to alphanumeric + symbols with escaped rendering.
