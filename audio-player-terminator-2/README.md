# Audio Player [Terminator 2: Main Theme]

Directory: `audio-player-terminator-2/`
...
## Live URL
https://numberdave-cloud.github.io/MPMPT1/audio-player-terminator-2/
...
```html
<iframe
  src="https://numberdave-cloud.github.io/MPMPT1/audio-player-terminator-2/"
  width="100%"
  height="190"
  title="Terminator 2 main theme audio player"
  style="border: none;"
  allow="autoplay"
  loading="lazy"></iframe>
```
...
## What it plays
Main Theme from Terminator 2 (1991). Brad Fiedel. YouTube ID `lqcHjUJadp8`.
...
- Fade config lives in three constants at the top of the script: `FADE_START` (95s = 1:35), `FADE_DURATION` (10s), `STOP_AFTER_FADE` (true, pauses the source once it reaches silence). Change these plus `VIDEO_ID` to reuse the player on another page or track.
- Progress bar scale is `FADE_START + FADE_DURATION` (105s = 1:45), not the real track length, so the bar fills to 100% at the exact moment the track goes silent. The time readout shows elapsed against that scale.
...
## Last updated
13 August 2026 (v1.0). Initial ship. One of a batch of fixed-track reading-bed players. Fade to silence over 10s starting at 1:35.
