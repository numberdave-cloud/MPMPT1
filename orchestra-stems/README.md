...
## How it works
- Silent on load. A single centered transport button toggles Play/Stop and drives all three stems together. Solo only sets what is audible.
...
- The three per-family volume faders (0 to 100%) are always visible below the transport.
- Corner label chip on each image names the family and stays visible on top of the text overlay.
- Layout is locked to three across at every width (no vertical fold-down). Overlay text sits on the image, so soloing does not change the layout height.

## Easter egg
Drag all three faders to 0, then tap the transport three times on/off (Play, Stop, Play, Stop, Play, Stop). On that final Stop the faders fade themselves up, the images ripple left to right into negative (CSS `invert(1)`), and reverse mode arms. The next Play runs all three stems backwards. Persists until the widget reloads.
...
## Last updated
2026-07-29: Removed Mixer button (faders now always visible); merged Play/Stop into one centered toggle; egg is now three on/off taps of that toggle at faders-0.
