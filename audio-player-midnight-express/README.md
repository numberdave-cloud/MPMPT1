# Audio Player [Midnight Express: The Chase]

Directory: `audio-player-midnight-express/`
...
## Live URL
https://numberdave-cloud.github.io/MPMPT1/audio-player-midnight-express/
...
```html
<iframe
  src="https://numberdave-cloud.github.io/MPMPT1/audio-player-midnight-express/"
  width="100%"
  height="190"
  title="Midnight Express The Chase audio player"
  style="border: none;"
  allow="autoplay"
  loading="lazy"></iframe>
```
...
## What it plays
The Chase from Midnight Express (1978). Giorgio Moroder. YouTube ID `mpW3C_k0WMY`.
...
- Fade config lives in three constants at the top of the script: `FADE_START` (116s = 1:56), `FADE_DURATION` (10s), `STOP_AFTER_FADE` (true, pauses the source once it reaches silence). Change these plus `VIDEO_ID` to reuse the player on another page or track.
- Progress bar scale is `FADE_START + FADE_DURATION` (126s = 2:06), not the real track length, so the bar fills to 100% at the exact moment the track goes silent. The time readout shows elapsed against that scale.
...
## Last updated
13 August 2026 (v1.0). Initial ship. One of a batch of fixed-track reading-bed players. Fade to silence over 10s starting at 1:56.
