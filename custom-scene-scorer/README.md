...
- Chrome crop: the video window is a cropped strip of the full 16:9 players. YouTube's top title/channel bar and bottom "More videos" bar sit at the player edges, so the players are enlarged and the window clipped to a shorter, slightly narrower shape to push that chrome out of frame. Both scene and music players are cropped identically via the same `.video-wrap` transform; the music player stays a large, viewport-intersecting element (visible strip well over 200px), so the sole-audio halt does not return. Two knobs at the top of the `.video-wrap` rule: `--frame-aspect` (window shape, 1.778 = full 16:9, higher = wider) and `--scene-zoom` (enlarge to push the bars out and pull the sides in, 1.0 = none). Currently 2.15 and 1.18, tuned by eye against the live embed; `--scene-shift` is an optional vertical nudge. The centre play button is intentionally left visible. Not currently using an opaque paused-state cover (as Energy Dial does); add one only if title chrome bleeds in on pause and the zoom cannot kill it.
- Load starts both players together: `handleLoad` pauses any current playback (no seek, so no autoplay quirk), sets `playAfterLoad`, then loads the bed. When the bed is ready, `afterMusicLoaded` calls `restart()` to seek both scene and music to the top and play them together. `play()` sets `isPlaying` before the state event, so the scene autoplay guard leaves this intentional play alone. Locked/example loads do not set `playAfterLoad`, so they open paused at the top. Note: because the swapped-in bed is freshly cued, there can be a small buffering lag before the music catches the scene, same as the normal PLAY button; tighten later by gating the play on the music's CUED state if it becomes noticeable.
...
## Last updated
2026-08-12 (later still) - LOAD now starts scene and music together instead of leaving them paused. Previously the scene autoplay guard paused the scene's seek-triggered play while the music (ungoverned) kept rolling, so LOAD played music with a still scene; now `afterMusicLoaded` calls `restart()` on a student LOAD to play both from the top. Earlier same day: stopped the scene autoplaying on page load (pause any student-untriggered play, keep the 0:37 frame via seek + guard); cropped YouTube chrome (`--frame-aspect` / `--scene-zoom`, 2.15 / 1.18); fixed the false error 150 with an 1800 ms grace window; added explicit `origin` playerVar; scene onError logging; reverted destroy/recreate; no-end-screen; muted scene; code-specific errors; initial ship.
