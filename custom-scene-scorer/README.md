...
## Last updated
2026-08-12 - Added explicit `origin` playerVar to all players to fix intermittent false error 150 from YouTube's referrer-based embedder-identity check (worked-then-failed-on-reload symptom). Added scene onError logging. Earlier same day: reverted destroy/recreate, no-end-screen, muted scene, code-specific errors, initial ship.
