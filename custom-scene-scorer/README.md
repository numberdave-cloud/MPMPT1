...
## Last updated
2026-08-12 (later still) - Stopped the scene autoplaying on page load. `seekTo()` on a not-yet-started player makes YouTube begin (muted) playback, so the scene rolled on load; `onSceneStateChange` now pauses the scene on any PLAYING event that the student did not trigger (`isPlaying` still false). The onReady seek to 0:37 is kept so the scene shows that frame rather than the poster: it lands paused on the 0:37 frame after a brief jump. Earlier same day: cropped YouTube chrome out of the scene window (tunable `--frame-aspect` / `--scene-zoom`, 2.15 / 1.18); LOAD hard-stops and rewinds both players; fixed the false error 150 on pasted-track swaps with an 1800 ms grace window; added explicit `origin` playerVar; scene onError logging; reverted destroy/recreate; no-end-screen; muted scene; code-specific errors; initial ship.
