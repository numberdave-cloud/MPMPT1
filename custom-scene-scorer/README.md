...
## Last updated
2026-08-12 (later) - Fixed the false error 150 on pasted-track swaps: it is a phantom YouTube throws during a `cueVideoById` swap and then immediately recovers from (isolated on a repro bench). Replaced the tear-down-on-150 handler with an 1800 ms grace window that only surfaces a real error if the player is still dead at the deadline; wired the music player `onStateChange` to catch recovery. The `origin` playerVar and referrer meta remain in place. Earlier same day: added explicit `origin` playerVar, scene onError logging, reverted destroy/recreate, no-end-screen, muted scene, code-specific errors, initial ship.
