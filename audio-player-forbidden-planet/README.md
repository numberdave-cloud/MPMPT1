# Audio Player [Forbidden Planet: Graveyard]

Directory: `audio-player-forbidden-planet/`
...
## Live URL
https://numberdave-cloud.github.io/MPMPT1/audio-player-forbidden-planet/
...
```html
<iframe
  src="https://numberdave-cloud.github.io/MPMPT1/audio-player-forbidden-planet/"
  width="100%"
  height="190"
  title="Forbidden Planet Graveyard audio player"
  style="border: none;"
  allow="autoplay"
  loading="lazy"></iframe>
```
...
## What it plays
Graveyard from Forbidden Planet (1956). Louis and Bebe Barron, the first fully electronic film score. YouTube ID `WR-MkDKWWW0`.
...
- Fade config lives in three constants at the top of the script: `FADE_START` (60s = 1:00), `FADE_DURATION` (10s), `STOP_AFTER_FADE` (true, pauses the source once it reaches silence). Change these plus `VIDEO_ID` to reuse the player on another page or track.
- Progress bar scale is `FADE_START + FADE_DURATION` (70s = 1:10), not the real track length, so the bar fills to 100% at the exact moment the track goes silent. The time readout shows elapsed against that scale.
...
## Last updated
13 August 2026 (v1.0). Initial ship. One of a batch of fixed-track reading-bed players. Fade to silence over 10s starting at 1:00.
