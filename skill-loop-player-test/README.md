...
```html
<iframe src="https://numberdave-cloud.github.io/MPMPT1/skill-loop-player-test/?v=2"
...
- The real player sits behind the video box with its chrome hidden (controls, keyboard, fullscreen off). The custom play button drives play and pause, the timeline scrubs, and Space and Escape work.
...
- Speed is a half-speed turtle toggle (`setPlaybackRate` 0.5 or 1). Volume slider and mute hit the player. iOS may ignore the volume slider since the OS owns volume there; mute works.
...
2026-06-11: replaced the staging build with the redesigned 2B layout, real player wired in, real per-loop notes, keycap-styled shortcuts.
