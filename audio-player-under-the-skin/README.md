# Audio Player [Under the Skin: Lonely Void]

Directory: `audio-player-under-the-skin/`
...
## Live URL
https://numberdave-cloud.github.io/MPMPT1/audio-player-under-the-skin/
...
```html
<iframe
  src="https://numberdave-cloud.github.io/MPMPT1/audio-player-under-the-skin/"
  width="100%"
  height="190"
  title="Under the Skin Lonely Void audio player"
  style="border: none;"
  allow="autoplay"
  loading="lazy"></iframe>
```
...
## What it plays
Lonely Void from Under the Skin (2013). Mica Levi. YouTube ID `7NIz94ILjxI`.
...
- Fade config lives in three constants at the top of the script: `FADE_START` (90s = 1:30), `FADE_DURATION` (10s), `STOP_AFTER_FADE` (true, pauses the source once it reaches silence). Change these plus `VIDEO_ID` to reuse the player on another page or track.
- Progress bar scale is `FADE_START + FADE_DURATION` (100s = 1:40), not the real track length, so the bar fills to 100% at the exact moment the track goes silent. The time readout shows elapsed against that scale.
...
## Last updated
13 August 2026 (v1.0). Initial ship. One of a batch of fixed-track reading-bed players. Fade to silence over 10s starting at 1:30.
