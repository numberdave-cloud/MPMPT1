...
```html
<iframe src="https://numberdave-cloud.github.io/MPMPT1/bpm-tap-v2/" width="100%" height="540" title="BPM Tap" style="border: none;" allow="autoplay" loading="lazy"></iframe>
```

Height 540 (non-default). The card measures ~512px at 700 to 900px wide and ~671px at 480px. Raise to 700 if the page is expected to be viewed on narrow phones.

## Build state

v2.1, 2026-09-08. Shipped as a new folder so the original `bpm-tap/` embed stays live untouched. v2.1 overwrote v2.0 in place (git history is the undo).
...
- TAP button rests transparent and flashes solid accent with a glow for 90 ms on hit. Spacebar also taps (repeat blocked); this is not advertised in the UI.
...
- Award panel with no empty-state copy; no footer hint.
...
2026-09-08. v2.1: title enlarged, subtitle / footer hint / award placeholder / BPM unit removed, borders thickened and brightened, type scaled 1.3x. Embed height raised to 540.
